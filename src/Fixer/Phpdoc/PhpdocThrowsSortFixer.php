...
namespace Sharoff45\Library\CodeStyleFixer\Fixer\Phpdoc;
...
final class PhpdocThrowsSortFixer implements FixerInterface
{
    ...
    {
        return 'Sharoff45/phpdoc_throws_sort';
    }
    ...
    private function sortThrowsAnnotations(string $content): string
    {
        ...
        $orderedThrowsAnnotationsContent = array_map(function (Annotation $annotation): string {
            return $annotation->getContent();
        }, $throwsAnnotations);

        usort($orderedThrowsAnnotationsContent, function (string $a, string $b): int {
            return $a <=> $b;
        });
